...
$$
\I_i = \tau_i + \lambda_i \ \F + \delta_i
$$

One of the assumptions of this model is the the error term $\delta_i \sim \mathcal N(0, \theta_i)$, or in other words, the error term has a mean of 0 and a variance of $\theta_i$. Through some complex math, we can show that the variance of each item is as follows:

$$
Var(\I_i) = \lambda_i^2 + \theta_i
$$
...
1.  Items with higher reliability are considered more "accurate" measures of the latent variable factor. They thus ensure better model estimation - and when a factor has low relaibility, we will often drop it.
...
$$
\text{factor scores} = \omega_0 + \omega_1 \ \I_1 + \omega_2 \  \I_2 + \dots
$$
