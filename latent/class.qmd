...
digraph example2 {
  bgcolor="transparent";
  // Nodes
  F [shape=box, pos = "0,0!", label="Latent Factor"]
...
$$
\begin{align}
& Pr(\text{item}_1 = \text{category }k_i \ | \ \text{factor} = \text{category }c)\\
& Pr(\text{item}_2 = \text{category }k_i  \ | \ \text{factor} = \text{category }c)\\
& Pr(\text{item}_3 = \text{category }k_i  \ | \ \text{factor} = \text{category }c)\\
\end{align}
$$
