...
$$
\begin{align}
\overbrace{\text{logit}[Pr(\I_1 = 1 \ | \ \text{factor})]}^{\text{logit}=\log[x/(1-x)]} & = \tau_1 + \lambda_1 \ \F \\
\text{logit}[Pr(\I_2 = 1 \ | \ \text{factor})] & = \tau_2 + \lambda_2 \ \F \\
\vdots \qquad \qquad \vdots \qquad \qquad \vdots & \qquad \qquad \vdots \\
\text{logit}[Pr(\I_p = 1 \ | \ \text{factor})] & = \tau_p + \lambda_p \ \F \\
\end{align}
$$
