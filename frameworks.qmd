...
| Unit $i$ does not Receive Treatment | $\text{treatment}_i = 0$ | $\purple{\text{outcome}_i^{(\text{untreated})}}$ |
| Unit $i$ Receives Treatment         | $\text{treatment}_i = 1$ | $\purple{\text{outcome}_i^{(\text{treated})}}$   |
...
$$
\tau_i = \purple{\text{outcome}_i^{(\text{treated})}} - \purple{\text{outcome}_i^{(\text{untreated})}}
$$
