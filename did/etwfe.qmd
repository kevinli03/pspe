...
$$
\begin{align}
\text{outcome}_{it} = & \sum\limits_{\text{group}} \sum\limits_t \blue{\tau_{\text{group},t}} \cdot 1\{i \in \text{group}\} \cdot \text{treatment}_{it} \\
+ & \sum\limits_\text{group} \sum\limits_t \sum\limits_j\blue{\tau_{\text{group},t,j}} \cdot 1\{i \in \text{group}\} \cdot \text{treatment}_{it} \cdot \text{covariate}_{j, it} \\ + & \  \text{unit}_i + \text{period}_t + \text{covariates}_{it} \b\beta + \eps_{it}
\end{align}
$$

::: small
The summations basically mean include one interaction for each group (initial treatment year group), and each $t$ (time periods) within each group. The $1\{\}$ are indicator functions that output a value of 1 if true, and value of 0 if not true. $i \in$ group means unit $i$ is in the initial treatment year group in question. The first line interacts treatment with initial group. The second line interacts treatment with initial group and covariates.
:::

This model will produce many coefficients $\blue{\tau_{\text{group}, t}}$ and $\blue{\tau_{\text{group}, t, j}}$ that capture different heterogeneity in treatment effects. To obtain a single ATT, we will want to aggregate them together in a weighted average.
