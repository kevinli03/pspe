...
& + \underbrace{\blue{\tau_b} \left( \text{group} \cdot \D \cdot \text{time} \cdot \cov \right)}_{\text{interactions for heterogeneity}} + \eps
\end{align}
$$
...
This model will produce many coefficients $\blue{\tau_a}$ and $\blue{\tau_b}$ that capture different heterogeneity in treatment effects. To obtain a single ATT, we will want to aggregate them together in a weighted average.
