...
$$
\begin{align}
\Y_{it} = & \ \blue{\tau_{\text{group}, t}} \cdot \text{group}_i \cdot \D_{it} \cdot \text{time}_t \\
& + \blue{\tau_{\text{group},t, \text{covariates}}} \cdot \text{group}_i \cdot \D_{it} \cdot \text{time}_t \cdot \cov_{it} \\
& +  \unit + \time + \beta \ \cov_{it} + \eps_{it}
\end{align}
$$

::: small
$\text{group}_i$ is a categorical variable that indicates what initial treatment period group unit $i$ belongs to. $\text{time}_t$ is a categorical variable that indicates the different time periods. $\text{covariates}_{\ it}$ is a vector of values for all covariates for a specific unit $i$ at period $t$. Essentially, the first line captures a interaction between groups, treatment, and time, and the second line captures an interaction between groups, treatment, time, and covariates.
:::
