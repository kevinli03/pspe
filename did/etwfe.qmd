...
$$
\begin{align}
Y_{it} = \alpha_i & + \gamma_t + \b X_{it}'\b\beta && \text{fixed effects and covariates}\\
& + \sum\limits_{g}\sum\limits_t \tau_{g,t} \cdot 1\{i \in g\} \cdot 1\{t ≥ g\} && \text{interactions with intial treat year} \\
& + \sum\limits_g \sum\limits_t \tau_{g,t,j} \cdot 1\{i \in g\}  \cdot X_{j, it} + \eps_{it} && \text{interact with covariates}
\end{align}
$$

::: small
The summations basically mean include one interaction for each $g$ (initial treatment year group), and each $t$ (time periods) within each $g$. The $1\{\}$ are indicator functions that output a value of 1 if true, and value of 0 if not true. $i \in g$ means unit $i$ is in the initial treatment year group of $g$.
:::
