...
$$
\begin{align}
Y_{it} = \sum\limits_{\text{group}} & \sum\limits_t \blue{\tau_{\text{group},t}} \cdot 1\{i \in \text{group}\} \cdot D_{it} \\
+ & \sum\limits_\text{group} \sum\limits_t \sum\limits_j\blue{\tau_{\text{group},t,j}} \cdot 1\{i \in \text{group}\} \cdot D_{it} \cdot X_{j, it} \\ + & \  \alpha_i + \gamma_t + \b X_{it} \b\beta + \eps_{it}
\end{align}
$$
...
This model will produce many coefficients $\tau_{\text{group}, t}$ and $\tau_{\text{group}, t, j}$ that capture different heterogeneity in treatment effects. To obtain a single ATT, we will want to aggregate them together in a weighted average.
