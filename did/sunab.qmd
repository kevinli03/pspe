...
$$
\begin{align}
\text{outcome}_{it} = & \ \blue{\tau_{\text{group}, \text{rel.time}}}  \cdot \text{group}_i \cdot \text{rel.time}_{it} \\
& + \text{unit}_i + \text{period}_t + \b{\text{covariates}}_{\ it} \b\beta + \eps_{it}
\end{align}
$$

::: small
$\text{group}_i$ is a categorical variable that indicates what initial treatment period group unit $i$ belongs to. $\text{rel.time}_{it}$ is a categorical variable that describes unit $i$ at time $t$, and how many periods away from the initial treatment period of that unit $i$ is. $\b{\text{covariates}}_{\ it}$ is a vector of values for all covariates for a specific unit $i$ at period $t$. Essentially, the first line interacts group with relative treatment time.
:::
