...
$$
Y_{it} = \alpha_i + \gamma_t + \b X_{it}'\b\beta + \underbrace{\sum\limits_g \sum\limits_{k ≠ -1} \tau_{g, k} \cdot 1\{i \in g\} \cdot 1\{t-g=k\}}_{\text{interactions for group-time ATT}} + \eps_{it}
$$

::: small
The summations mean include one interaction for each $g$ (initial treatment year group), and each $t$ (time periods) within each $g$. The $1\{\}$ is an indicator function that takes a value of 1 if true, and 0 if false. $i \in g$ means unit $i$ initially recieved treatment in period $g$. $t-g$ represents the relative year of treatment, with the year treatment initiates is year 0.
:::
