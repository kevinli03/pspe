...
In April 1992, New Jersey increased its minimum wage. We want to know how this affected employment rates. We have data from before and after the change:
...
With this data, we **cannot** find the causal effect. Why? Well recall our definition of the causal effect using potential outcomes. Our treatment effect after the treatment in December 1992 would be:

$$
\tau_\text{Dec92} = \purple{\text{outcome}^{(\text{treated})}_\text{Dec92}} - \red{\text{outcome}^{(\text{untreated})}_\text{Dec92}}
$$
...
\begin{align}
\tau_\text{Dec92} &  = \purple{\text{outcome}^{(\text{treated})}_\text{Dec92}} - \red{\text{outcome}^{(\text{untreated})}_\text{Dec92}} \\
& \approx \purple{\text{outcome}^{(\text{treated})}_\text{Dec92}} - \left( \purple{\text{outcome}^{(\text{untreated})}_\text{Mar92}}  - 2.16\right)
\end{align}
$$
