...
For example, let's assume that intellegence has two values: smart and dumb. Let us calculate the treatment effect on university grades within each level of intellegence:

$$
\begin{align}
\tau_\text{smart} & = \purple{\text{grades}_\text{smart}^{(\text{got scholarship})}} - \purple{\text{grades}_\text{smart}^{(\text{no scholarship})}}\\
\tau_\text{dumb} & = \purple{\text{grades}_\text{dumb}^{(\text{got scholarship})}} - \purple{\text{grades}_\text{dumb}^{(\text{no scholarship})}}
\end{align}
$$
